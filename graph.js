...
History.prototype = {
    ...
    _connection: {
        ...
        straight: function(history, i, back) {
            for (var l = history.length; i < l; ++i) {
                this.ctx.lineTo(i - 0.5, history[i - 1] + (this.last[i - 1] || 0));
                this.ctx.lineTo(i - 0.5, history[i] + (this.last[i] || 0));
                this.ctx.lineTo(i, history[i] + (this.last[i] || 0));
            }
            if (this.last.length && back) {
                this.ctx.lineTo(--i, this.last[i]);
                for (--i; i >= 0; --i) {
                    this.ctx.lineTo(i + 0.5, this.last[i + 1]);
                    this.ctx.lineTo(i + 0.5, this.last[i]);
                    this.ctx.lineTo(i, this.last[i]);
                }
            }
        },
        curve: function(history, i, back) {
            for (var l = history.length; i < l; ++i) {
                this.ctx.curveTo(i - 0.5, history[i - 1] + (this.last[i - 1] || 0), i - 0.5, history[i] + (this.last[i] || 0), i, history[i] + (this.last[i] || 0));
            }

            if (this.last.length && back) {
                this.ctx.lineTo(--i, this.last[i]);
                for (--i; i >= 0; --i) {
                    this.ctx.curveTo(i + 0.5, this.last[i + 1], i + 0.5, this.last[i], i, this.last[i]);
                }
            }
        }
    },
